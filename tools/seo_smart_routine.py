...
def app_bottomnav(active=None):
    ...
    # same fix as index.html's own nav and generate_seo_pages.py's copy of
    # this function. Routed through a() like its siblings.
...
def _active_tab_for(canonical):
    if canonical == '/klima/':
        return 'klima'
    if canonical.startswith('/novosti/'):
        return None
    return 'arhiv'
...
def article_schema(url, title, desc, date_pub, date_mod=None):
    ...
    return (f'<script type="application/ld+json">\n'
            ...
            f'"author":{{"@type":"Person","name":"Filip Eremita","url":"{SITE}/o-postaji.html","sameAs":["https://ibanezar.github.io","https://www.wunderground.com/dashboard/pws/IREICA1"]}},'
...
{app_bottomnav(_active_tab_for(canonical))}
</body>
</html>'''
