...
(function(){
  ...
  // Best index anywhere in the 7-day window for this forest — null if today
  // (index 0) already is the peak, so the row doesn't state the obvious.
  function bestDayText(loc){
    var days=loc.days,bi=0;
    for(var i=1;i<days.length;i++){if(days[i].overall>days[bi].overall)bi=i;}
    if(bi===0)return null;
    return dayLabel(days[bi],false)+' · '+days[bi].overall+' %';
  }
  ...
  function render(d){
    ...
    var html="";
    // today per forest — richer than the free list: top-3 species (not just
    ...
    html+='<h3>Danes po gozdovih</h3><div class="gp-forests-premium">';
    locs.slice().sort(function(a,b){return b.days[0].overall-a.days[0].overall;})
      .forEach(function(l){
        ...
        var metaHtml=(o.soil_moisture_pct==null?'':'<span>💧 vlaga tal '+o.soil_moisture_pct+' %</span>')+
          (bestDayText(l)?'<span>📈 najboljši dan: '+bestDayText(l)+'</span>':'<span>🔝 danes je vrh tedna</span>');
        html+='<div class="gp-forest gp-forest-premium"><div class="gp-forest-info">'+
